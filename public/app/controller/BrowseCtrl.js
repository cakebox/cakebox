app.controller('BrowseCtrl', ['$scope', '$routeParams', 'breadcrumbs', 'Directory', 'File',
    function($scope, $routeParams, breadcrumbs, Directory, File) {
        ...
        $scope.archiveDirectory = function(directory) {
            alertify.log("L'archive " + directory.name + ".tar est en cours de création, veuillez patienter.", "success", 60000);

            Directory.archive({'path': $scope.currentPath + directory.name}, function(data) {
                alertify.log("L'archive " + directory.name + ".tar a bien été créée.", "success", 60000);
            }, function(error) {
                if(error.status == 403) {
                    alertify.log("Le repertoire de destination n'a pas les droits necessaires.", "error", 60000);
                }
                $scope.informations = "Erreur " + error.status + " (" + error.statusText + "): " + error.config.method + " " + error.config.url;
            });
        };
        ...
        $scope.removeDirectory = function(directory) {
            Directory.delete({'path': $scope.currentPath + directory.name}, function(data) {
                alertify.log("Le dossier " + directory.name + " est bien supprimé.", "success", 60000);
                retrieveDirectories($scope.currentPath);
            }, function(error) {
                if(error.status == 403) {
                    alertify.log("Le dossier " + directory.name + " n'a pas les droits necessaires pour être supprimé.", "error", 60000);
                }
                $scope.informations = "Erreur " + error.status + " (" + error.statusText + "): " + error.config.method + " " + error.config.url;
            });
        };
        ...
        $scope.removeFile = function(file) {
            File.delete({'path': $scope.currentPath + file.name}, function(data) {
                alertify.log("Le fichier " + file.name + " est bien supprimé.", "success", 60000);
                retrieveDirectories($scope.currentPath);
            }, function(error) {
                if(error.status == 403) {
                    alertify.log("Le fichier " + file.name + " n'a pas les droits necessaires pour être supprimé.", "error", 60000);
                }
                $scope.informations = "Erreur " + error.status + " (" + error.statusText + "): " + error.config.method + " " + error.config.url;
            });
        };
